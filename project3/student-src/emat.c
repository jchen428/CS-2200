...
double compute_emat() {
    ...
    //long int total_time = MEMORY_ACCESS_TIME * (total_accesses + tlb_misses) + DISK_ACCESS_TIME * count_pagefaults;
    long int total = MEMORY_ACCESS_TIME * (count_tlbhits + 2 * tlb_misses) + (2 * MEMORY_ACCESS_TIME + DISK_ACCESS_TIME) * count_pagefaults;
    
    return total / (double)total_accesses;
}

...
main:       la $sp, stack				! load address of stack label into $sp
            ...
            addi $a0, $zero, 4 	        ! $a0 = 8, the fibonacci argument
...
fibonacci:  add $fp, $sp, $zero 		! set frame pointer to stack pointer
			...
			add $s0, $a0, $zero			! save original argument (n) in $s0
			addi $a0, $a0, -1			! store new argument n - 1 in $a0
			sw $a0, 0x1($fp)			! save n - 1 on stack
			addi $s1, $a0, -1			! store new argument $a0 - 1 in $s1 (n - 2)
			sw $s1, 0x2($fp)			! save n - 2 on stack

			addi $s2, $zero, 1			! set $s2 to 1 to check if $a0 == 1
			beq $s0, $zero, if_zero		! if $s0 (n) == $a1 (0) then go to if_zero
			beq $s0, $s2, if_one		! if $s0 (n) == $a2 (1) then go to if_one
			...
			lw $s1, 0x3($fp)			! load first result into $s1 from stack
			add $v0, $s1, $v0			! store fib(n - 1) + fib(n - 2) in $v0 (second result is already in $v0)
